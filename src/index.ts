...
import { handleArbitrationButton, handleArbitrationModal } from './lib/arbitration.js';
...
client.on('interactionCreate', async (interaction) => {
  try {
    if (interaction.isButton()) {
      const handled = await handleArbitrationButton(interaction);
      if (handled) return;
    }

    if (interaction.isModalSubmit()) {
      const handled = await handleArbitrationModal(interaction);
      if (handled) return;
    }
    ...
    if (interaction.isRepliable()) {
      const response = { content: '❌ Une erreur est survenue lors du traitement de l’interaction.', ephemeral: true } as const;
